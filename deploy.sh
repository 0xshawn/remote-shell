#!/usr/bin/env bash
# One-command first deploy: build + start the stack, auto-wire the host SSH key,
# and surface the login password. Safe to re-run (idempotent). Run from anywhere;
# it operates on the repo this script lives in.
set -euo pipefail
