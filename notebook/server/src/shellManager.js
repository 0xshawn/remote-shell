...
const PREFIX = 'nb_'; // namespace for all tmux sessions owned by notebook
...
// ShellManager
//
// Persistence model (identical to remote-shell):
...
class ShellManager {
  ...
  tmuxExists(key) {
    try {
      // `=key` forces an EXACT match (avoids nb_a_1 matching nb_a_10).
  ...
  attach(user, sessionId, { cols, rows }, ws) {
    ...
    const term = pty.spawn('tmux', args, {
      ...
      rows: r,
      cwd: this.config.cwd,
      // Force a UTF-8 locale for the tmux client so CJK / wide chars render.
  ...
  killSession(key) {
    ...
    logger.info(`killed shell session ${key}`);
  }
  ...
  // List notebook-owned shell sessions straight from tmux (survives server restart).
  list() {
    try {
      ...
      return out.split('\n').filter(Boolean).reduce((acc, line) => {
        ...
        if (!name || !name.startsWith(PREFIX)) return acc;
        acc.push({ key: name, createdAt: parseInt(created, 10) * 1000 || 0, attached: parseInt(attached, 10) > 0 });
        return acc;
      }, []);
    } catch {
      return [];
