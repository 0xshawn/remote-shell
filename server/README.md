...
the background service with zero network access:

```bash
bash remote-shell-installer-linux-amd64.sh
```

Or, with just the raw binary: `./install-binary.sh ./remote-shell-linux-amd64`
(also accepts `REMOTE_SHELL_BIN=/path/to/binary`).
